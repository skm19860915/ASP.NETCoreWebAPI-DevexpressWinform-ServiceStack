...
{
    public class MomoRemittanceService : IMomoRemittanceService
    {
        private readonly IMomoRemittanceHandler _restService;
        private string _authorization;
        private readonly string _remittanceUserId;
        private readonly string _remittanceUserSecretKey;
        private readonly string _remittanceSubscriptionKey;
        private readonly ILogger<MomoRemittanceService> _logger;
        private string _momoEnvironment;
        private const int RetryCount = 5;

        public MomoRemittanceService(AppConfig config, ILoggerFactory loggerFactory)
        {
            var paymentSettings = config.MomoPaymentSettings;
            _momoEnvironment = config.MomoPaymentSettings.Environment;
            _restService = RestService.For<IMomoRemittanceHandler>(paymentSettings.BaseUri);

            _remittanceUserId = paymentSettings.Remittance.UserId;
            _remittanceUserSecretKey = paymentSettings.Remittance.UserSecretKey;
            _remittanceSubscriptionKey = paymentSettings.Remittance.SubscriptionKey;
            CreateBase64Credentials();

            _logger = loggerFactory.CreateLogger<MomoRemittanceService>();

        }

        private void CreateBase64Credentials()
        {
            var creds = string.Join(":", _remittanceUserId, _remittanceUserSecretKey);
        ...
        {
            var response = await _restService.GetAccessToken(_remittanceSubscriptionKey, _authorization);
        ...
        public async Task<Guid> MakeTransfer(RequestPayee requestPayee)
        {
            ...
            await _restService.MakeTransfer(_remittanceSubscriptionKey, authorization, _momoEnvironment, requestReference.ToString(), requestPayee);
        ...
        public async Task<TransferResponse> GetTransferStatus(Guid requestReference)
        {
            ...
            var response = await _restService.GetTransferStatus(_remittanceSubscriptionKey, authorization, _momoEnvironment, requestReference);
        ...
        private async Task<AccountBalance> GetAccountBalanceRetry()
        {
            ...
            var response = await _restService.GetAccountBalance(_remittanceSubscriptionKey, authorization, _momoEnvironment);
